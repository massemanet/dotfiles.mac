...
state() ->
  {_,Dict} = process_info(whereis(htop),dictionary),
...
init(_) ->
  ...
  prf:start(htopConsumer,node(),htopConsumer),
  prf:config(htopConsumer,consumer,{client,self()}),
...
handle_info(?HTTP_GET(Socket,Item),LD) ->
  case answer(Item,LD) of
    ...
    postpone ->
      %% if the timer expires, close the socket
      %% if the socket closes, cancel the timer and close the socket
      %% if the item arrives, cancel the timers, send and close the sockets
      Timer = socket_timer(Socket),
      pd_append({item,Socket},{Item,Timer}),
      pd_append({sockets,Item},Socket),
      LD;
    close ->
      close(Socket,LD)
  end;

handle_info({timeout,Timer,Socket},LD) ->
  [{Item,Timer}] = get({item,Socket}),
  ?log({unrecognized_request,Item}),
  close(Socket,LD);

handle_info([{prfPrc,PrfPrc},{prfSys,PrfSys}],LD) ->
  Node = to_str(get_value(node,PrfSys)),
  handle_data([{procs,make_procs(PrfSys,PrfPrc)},{prfSys,PrfSys}],Node,LD);

handle_info({watchdog,_Node,ticker,Info},LD) ->
  handle_info(Info,LD);

handle_info({watchdog,_,_,_},LD) ->
  LD;
...
socket_timer(Socket) ->
  erlang:start_timer(3000,self(),{timeout,Socket}).
...
handle_data(Data,Node,LD) ->
  send_data(Data,Node++"/prf",LD).
  
send_data(Proplist,Item,LD) -> 
  Sockets = pd_clear({sockets,Item}),
  case Sockets of 
    [] -> LD;
    _ -> 
      Json = (LD#ld.encoder)(structify(Proplist)),
...
close(Socket,LD) ->
  ...
  case pd_clear({item,Socket}) of
    [{Item,Timer}] -> erlang:cancel_timer(Timer),
                      pd_erase({sockets,Item},Socket);
    [] -> ok
  end,
  LD.
...
all([F|Fs],O) -> 
  NO = try F(O) 
       catch C:R -> ?log_bt({C,R}),O 
       end,
  all(Fs, NO).

%% use process dict to get better scaling than proplists
pd_append(Key,Val) -> 
  case put(Key,[Val]) of
    undefined -> [];
...
pd_erase(Key,Val) ->
  case pd_clear(Key) of
    []    -> [];
    [Val] -> [];
    Vals  -> put(Key,Vals--[Val])
  end.
